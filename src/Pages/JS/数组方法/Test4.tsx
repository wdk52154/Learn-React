...
const Test4: React.FC = () => {
  ...
  //3 filter 过滤,当return后面bool值为true的时候,会生成一个新数组，返回当前元素.没有返回条件的,会返回空数组.
  //   const arr = [4, 5, 6, 7, 8];
  //   const res = arr.filter(function (v, k) {
  //     // console.log(v); //数组的每一个元素
  //     // console.log(k); //索引
  //     return v > 7;
  //   });
  //   console.log(res); //[8]
  ...
  //4 find 返回符合条件的元素(断言函数,找到目标则停止循环)
  ...
  //5 findIndex  返回符合条件元素的下标(断言函数,找到目标则停止循环)
  //   const ages = [3, 15, 20, 30];
  ...
  //6 includes  是否包含某个元素,是返回true,不是返回flase.
